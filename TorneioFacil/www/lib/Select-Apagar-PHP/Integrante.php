...
<head>
    ...
    <title>Deletar Integrante </title>
</head>
<body>
...
if( isset($_POST['BotaoEnviar']) ){
    ...
    $query_resultado = "SELECT * FROM integrante WHERE cpf = $id LIMIT 1";
    $resultado = $pdo->prepare($query_resultado);
    $resultado->execute();

    if( ($resultado) AND ($resultado->rowCount() != 0) ){
    $remover = "DELETE FROM integrante WHERE  cpf = $id";
    $apagar = $pdo->prepare($remover);

    if( $apagar->execute() ){
        echo "<script>alert('Integrante apagado com sucesso!');</script>";
    }else{
        echo "<script>alert('Erro: Integrante não apagado!');</script>";
    }
